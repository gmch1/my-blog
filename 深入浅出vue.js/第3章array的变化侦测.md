...
因为vue.js的实现方式决定了无法对上面举的两个例子做拦截，也就没有办法响应。在ES6之前，无法做到模拟数组的原生行为，所以拦截不到也是没有办法的事情。ES6提供了元编程的能力，因而有能力拦截，所以vue3.0中使用proxy来实现这部分功能，从而解决这个问题。
...
为了不污染全局Array.prtototype，我们在Observer中只针对那些需要侦测变化的数组使用\_\_protot\_\_来覆盖原型方法，但是\_\_protot\_\_在ES6之前并不是标准属性，不是所有llq都支持它。因此针对不支持\_\_protot\_\_属性的浏览器，我们直接循环拦截器，把拦截器中的方法直接设置到数组身上来拦截Array原型上的方法。

Array收集依赖的方式和Object一样，都是在getter中收集。但是由于使用依赖的位置不同，数组要在拦截器中向依赖发送消息，所以依赖不能向Object那样保存在defineReactive中，而是保存在了Observer实例上。
